...
\begin{document}
\qtreecenterfalse
\shorttitlerunninghead{On the lack of φ-\isi{feature resolution} in DP coordinations}
\maketitle
...
\ea\label{baseline-inanim} \textbf{gender ({\textsc{mi}/\textsc{f}}) $\succ$ \isi{neuter} (\textsc{n})}
...
\noindent One could argue that \isi{neuter} cannot participate in a syntactic resolution because it is in some sense defective. Such a conclusion goes in line with the following observation. Not only does \isi{neuter} never win in a combination with other gender values, but neuter-\isi{plural agreement} arises only if both conjuncts are in \isi{neuter} \isi{plural}, as shown in \REF{neuter}. If either or both of the conjuncts are in \isi{neuter} singular, the \isi{plural agreement} cannot be in \isi{neuter} \isi{plural}, despite the fact there is a dedicated \isi{neuter} \isi{plural agreement} morphology. Instead, the agreement is the syncretic gender agreement. 
...
One could dismiss the emergence of the \isi{masculine} \isi{animate} \isi{plural agreement} as insignificant, if it was not for an additional and much more serious empirical problem. The generalization reported in the literature are strictly based on examples in the \isi{past tense}. The \isi{past tense} in \ili{Czech} is morphologically realized by a finite auxiliary that agrees in person and number and is null for 3rd person and a past \isi{participle} that agrees in number and gender with a structural subject in \isi{nominative} case. Strikingly, the feature-resolution generalization reported in the previous section does not extend to other constructions in which we see \isi{plural agreement} in gender, i.e., agreement with \isi{adjectival} predicates and passive participles. 

Agreement with \isi{adjectival} predicates and passive participles plays out rather differently. As it turns out, if the gender features on conjuncts do not match, \isi{plural agreement} is fully grammatical only if one \isi{conjunct} is \isi{masculine} \isi{animate} and the other \isi{conjunct} is grammatically \isi{feminine} but may be semantically construed as \isi{animate}, as in \REF{anim-2}.\footnote{The consistency of \isi{masculine} \isi{animate} agreement in these patterns have been confirmed in \citet{Adam:2017}, a large scale ($N={}$103) elicitation study testing some of the data from an unpublished version of this paper. Adam tested only \isi{animate} coordinations and confirmed that whenever one of the conjuncts is \isi{masculine} \isi{animate}, \isi{plural agreement} is \isi{masculine} \isi{animate}, irrespective of the order of the conjuncts.}
...
\noindent If there is no \isi{masculine} \isi{animate} gender, grammatically \isi{inanimate} gender combinations are strongly degraded even if they semantically denote \isi{animate} objects. When the coordination contains an \isi{inanimate} \isi{masculine} \isi{noun} and a \isi{neuter}, the expected agreement, i.e., \isi{masculine} \isi{inanimate} (syncretic with \isi{feminine} \isi{plural}), is stongly degraded, \REF{inam}. Speakers I tested strongly preferred colloquial morphology (Common \ili{Czech}), which is completely syncretic in \isi{plural}, i.e., no gender or animacy distinction is preserved in the system (e.g., \citealt[76]{KarlikEtAl:2002}), (\ref{colloq1}).
...
As for the combination of \isi{feminine} and \isi{neuter}, no agreement pattern is fully acceptable either. In a forced written elicitation task reported in \citet{Adam:2017}, speakers volunteered \isi{feminine} \isi{plural} (62\%), i.e., the prescriptively required agreement, \isi{neuter} \isi{plural} (about 26\%), i.e., syncretic \isi{plural} in some dialects, or colloquial morphology (12\%), i.e., fully syncretic agreement, (\ref{inam-fem}). In my original data collection which was based on a spoken elicitation and a grammatical judgement task, speakers found the colloquial ending most acceptable.\footnote{\citeauthor{Adam:2017}'s study was based on data reported in the 2017 manuscript version of this paper. The judgements reported here thus reflect her finding. \citeauthor{Adam:2017} didn't test any of the other feature combinations as her focus was on \isi{animate} agreement and agreement with numerals.}
...
In a nutshell, I argue that the label of the \isi{conjunction phrase} is determined in the \isi{syntax-\isi{semantics} interface}, and the labelling process is analogical to the \isi{feature resolution} attested in split-antecedent pronouns (\citealt{Heim2008}, \citealt{Sudo2012}), i.e., \isi{plural} pronouns that simultaneously refer to more than one antecedent (e.g., \textit{you} and \textit{I} gives \textit{we}), provides an explicit algorithm for how the features of the referring antecedent are computed from a mixed-feature input. In the present proposal, the actual agreement is then modelled as a narrow-syntax agree that targets the conjunction-phrase label as the syntactic representation of the \isi{conjunction phrase}, where label is a syntactic representation of all features present in the corresponding extended projection and relevant for next syntactic building. There are three components: First, agree is successful only if the label provides features that match the features of the probe. Second, following \citet{Sudo2012}, I assume that the \isi{syntax-\isi{semantics} interface} manipulates semantic indices (i.e. numerical pointers). Crucially, indices are complex structures, enriched by person, gender, and number information. Third,  this complex-index information can be mapped onto morphology. Fourth, morphology can only realize features uniquely determined and valued by the label of the probe. The consequence is that if agree probes for person, agreement reflects the complex features of the indices. If agree probes for gender, it can only use gender features available to the \isi{narrow syntax} component. In other words, while \isi{semantics} can build new objects (complex indices), syntax can only copy existing values of features. Consequently, if agree probes for person, it can used the complex structures built by the \isi{syntax-\isi{semantics} interface}. If agree probes for gender, it may only use features already present in \isi{narrow syntax}.

\subsection{Features of the conjunction-phrase label}

The idea that there is a connection between agreement with coordinated \isi{noun} phrases and features of split-antecedent pronouns is intellectually indebted to \citet{Farkas1995} that proposed a striking generalization, namely, that the morphological features on the predicate agreeing with a nominal conjunction are always identical to the morphological features of a \isi{pronoun} anaphorically referring to the same coordination. To implement this idea I follow \citet{Heim2008} in her treatment of split-antecedent pronouns and \citet{Sudo2012} in his treatment of complex indices underlying the morphological representation of split-antecedent pronouns. 
...
\noindent Let us now consider the next case which is the case when there is a [$+$\textsc{person}] feature but no [$+$\textsc{participant}] feature in the label of the \isi{conjunction phrase}. This case arises if none of the conjuncts is syntactically valued as \isi{masculine} \isi{animate} but one or both conjuncts are syntactically valued as \isi{masculine} \isi{inanimate} or \isi{feminine}. Since the label is marked as [$-$\textsc{participant}], the morphological realization resorts to the gender marking in the context of \isi{plural}, i.e., the syncretic morphology for masculine-\isi{inanimate} \isi{plural} and \isi{feminine} \isi{plural}. This feature combination is exemplified by \REF{baseline-inanim}, repeated below as \REF{baseline-inanim-rep}.
...
\noindent The problem we just identified lies in the combinatorics behind the labelling operation. There is a caveat though. While the probe in these cases needs to be valued for person, it morphologically realizes gender features. Which is to say, if the label is uniquely labelled for gender from syntax, then morphology could realize the \isi{gender feature} in and of itself. However, I argue that this may happen only if the syntactic features on both conjuncts are identical, i.e., only if \isi{narrow syntax} provides \textsc{n.pl} as the common feature of the conjuncts. If this is the case, no feature calculation is necessary and the system solely copies the \isi{neuter} \isi{plural} label of its parts into the label of the \isi{conjunction phrase} and this information determines the morphological mapping of the resulting agreement as \isi{neuter} \isi{plural}. This is the pattern we saw in \REF{n-c}, repeated below as \REF{n-c-rep}.
...
The core property of the system proposed in the previous section is that agreement with coordinated \isi{noun} phrases is always mediated by the label of the \isi{conjunction phrase}. Crucially, we saw that some agreement combinations cannot be resolved because of a problem with valuation of the agree probe because the label of the \isi{conjunction phrase} has not been uniquely resolved. Interestingly, in the domain of agreement gaps, we saw that even if there is a good morphological match, the lack of successful valuation yields agreement failure. Consequently, if this reasoning is correct, we expect to find problems with valuation elsewhere. This section investigates two empirical domains that confirm this prediction.

Let us start with agreement gaps. If agreement gaps result from problems of labelling, i.e., from the fact there is no unique feature in the label that could value an unvalued feature of the probe, we expect to find agreement gaps elsewhere. This prediction is born out in comitative constructions and first-\isi{conjunct} agreement constructions. Although in comitative constructions only one \isi{conjunct} is in \isi{nominative}, agreement is with both conjuncts. Which means the agreement must be based on the features of the label of the \isi{conjunction phrase}. Consequently, we expect agreement gaps to arise exactly in the same environment as with regular coordinated phrases. Which is to say, we expect agreement gaps whenever the probe does not probe for person but only for gender, and whenever the conjuncts do not share gender features or are not both marked as [$+$\textsc{participant}]. This prediction is borne out, as can be seen, for example, in \REF{comin-gap}.
...
We know, however, that the variation in agreement goes beyond morphological \isi{syncretism}. The dialects fundamentally vary in their semantic index representation, as attested by differences in binding. Consider the example in \REF{binding}.
...
ConjP&\isi{conjunction phrase}\\
\textsc{f}&\isi{feminine}\\
\textsc{instr}&\isi{instrumental}\\
\textsc{m}&\isi{masculine}\\
\textsc{ma}&\isi{masculine} \isi{animate}\\
\textsc{mi}&\isi{masculine} \isi{inanimate}\\
\end{tabularx}%
\begin{tabularx}{.5\textwidth}{@{}lQ@{}}
\textsc{n}&\isi{neuter}\\
