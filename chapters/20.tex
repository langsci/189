...
% \begin{styleHeading}
% Adverbial puzzle in simultaneous readings of present and past-under-past in \ili{Russian}
% \end{styleHeading}
...
\noindent The simultaneous reading of past-under-past in \REF{20:ex1} can be enforced by some adverbials, such as \textit{togda} ‘then’.\footnote{\label{20:fn3}In this respect, \ili{Russian} behaves like \ili{Hebrew}, as it was reported in \citet{OgiharaSharvit2012}.} In \REF{20:ex2} \textit{togda} anaphorically refers to 2016 and the interpretation where the event of saying and the state of being the president of Russia overlap in time becomes the most salient.
...
\noindent If past-under-past and present-under-past can get the same interpretation, why is \textit{then} possible in the embedded \isi{clause} in the first case, but not in the second?

On the other hand, there are some adverbials, such as \textit{sejčas} ‘now’, that are compatible with present-under-past in \ili{Russian} \REF{20:ex5}, but not with past-under-past \REF{20:ex6}. Thus, the presence of \textit{sejčas} in \REF{20:ex6} makes it ill-formed, whereas without \textit{sejčas} both sentences \REF{20:ex5} and \REF{20:ex6} can have the simultaneous reading.\footnote{\label{20:fn6}I do not translate \REF{20:ex5} into \ili{English} as `When I talked to her 3 years ago, Tanja told me that she is pregnant now' because this \ili{English} sentence does not have the relevant reading due to the fact that there is no relative present in \ili{English} and \textit{now} is indexical, unlike \textit{sejčas} `now'.}
...
I will introduce my assumptions about the structure of the embedded clauses and the relative present in \ili{Russian} and will show how the \isi{semantics} of \textit{togda} presented here correctly predicts the restrictions on its use in embedded contexts.

For the simultaneous reading of past-under-past I will adopt the classical \textit{de re} approach \citep{Abusch1997,Heim1994}. I will show that the \isi{presupposition} of \textit{togda} that I am introducing is weak enough to make it compatible with the simultaneous reading of past-under-past.

In \sectref{20:s3} I will show that the \textit{de re} analysis of the simultaneous reading of past-under-past incorrectly predicts that \ili{Russian} \textit{sejčas} ‘now’ should be able to appear in such a context. Since under the \textit{de re} analysis the tense moves out of the embedded \isi{clause} and is interpreted separately from the \isi{adverbial}, no meaning clash is predicted between the \isi{past tense} and the present-oriented \isi{adverbial} \textit{sejčas}. I will propose that this problem can be solved if we adopt an assumption that a tense and an \isi{adverbial} are interpreted together. Since, when \isi{past tense} gets the simultaneous reading under past in believe/say contexts, it is interpreted outside of the embedded \isi{clause}, the \isi{adverbial} \textit{sejčas} has to be interpreted outside of the embedded \isi{clause} as well.
...
\noindent In principle, \textit{togda} can pick an interval that is inside the interval denoted by ‘this year’ as it is shown in \REF{20:ex15}, where it occurs with the \isi{past tense} embedded under past. In \REF{20:ex15} \textit{togda} anaphorically refers to the time when Tanja pronounced the words.
...
I will start this discussion by ruling out the simple idea that \ili{Russian} \textit{togda} requires \isi{past tense} in the same \isi{clause} to be licensed. One implementation of such an idea would be that \textit{togda} has to agree with \isi{past tense} and the agreement relation can only be established locally.

In \ili{Russian}, there are several adverbials that have a meaning similar to \textit{togda} and can occur in subordinate clauses with \isi{past tense} embedded under past. They are listed in \REF{20:ex16}. The fact that all of them are good with past-under-past is shown in \REF{20:ex17}.
...
\noindent All of them are infelicitous with \isi{present tense} embedded under past, as it is shown in \REF{20:ex18}.
...
\noindent A strong argument against the hypothesis that all of those elements have to be licensed by \isi{past tense} comes from the fact that all of them are in fact compatible with future tense in the same \isi{clause}. One example where \textit{togda} occurred in a \isi{matrix clause} with a future tense was given in \REF{20:ex8}. In \REF{20:ex19} I show that all of the adverbials given in \REF{20:ex16} are compatible with an embedded future. The antecedent for \textit{togda} or \textit{na tot moment} ‘at that moment’ in \REF{20:ex19} is given in a previous sentence and the resulting sentence is well-formed.
...
\noindent We can conclude that it is not the case that \textit{togda} (as well as other anaphoric elements that are compatible with past-under-past and incompatible with present-under-past) needs to be licensed by the \isi{past tense} in the same \isi{clause}.
...
\noindent A stronger \isi{presupposition} that would also prevent \textit{togda} from picking the time interval denoted by ‘this year’ would be that the time interval it picks does not overlap with the evaluation time. However, this would incorrectly predict that \textit{togda} is incompatible with the simultaneous reading of past-under-past.
...
\noindent I will adopt the pronominal \isi{semantics} for tenses \citep{Partee1973}. Tenses carry indices, thus, like other pronouns, they get their denotation via the assignment function $g$. The \isi{semantics} for \isi{present tense} that I will assume is given in \REF{20:ex27}. It simply denotes a specific time interval and presupposes that this time interval is equal to the evaluation time.
...
The denotation for the relative present is given in \REF{20:ex29}: essentially it has the same denotation as the regular present in \ili{Russian}.
...
One natural question arising at this point is whether the presence of this \isi{presupposition} is predicted to block the use of \textit{togda} in simultaneous reading of past-under-past as well because this would not be the desired result, as the well-formedness of \REF{20:ex2} shows. This is the question I will address in the next subsection.
...
Thus, I will assume that when \textit{togda} is acceptable with the simultaneous reading of past-under-past, the simultaneous reading is derived via the \textit{de re} \isi{construal}.
...
\noindent The intension of the predicate of times is computed in \REF{20:ex39}. In this system, the time of Putin’s presidency (in Tanja’s say-alternatives) and the local evaluation time are two distinct times. \textit{Togda} contributes the \isi{presupposition} that those two intervals are not equal to each other.\footnote{\label{20:fn11}The full \textit{de re} analysis requires another \isi{presupposition} in the embedded \isi{clause} that the time of the state or eventuality described in the embedded \isi{clause} is not in the future with respect to the local evaluation time (the upper limit constraint; cf. \citealt{Abusch1997}). The full intension of the embedded \isi{clause} is shown in \REF{20:fnex1}. This \isi{presupposition} is responsible for the fact that past-under-past cannot have the forward shifted reading.
...
Since the temporal \textit{de re} \isi{construal} derives the simultaneous reading of past-under-past without requiring that the two time intervals are exactly equal, the \isi{presupposition} that \textit{togda} carries is not going to be harmful for the meaning of the sentence. Thus, the \isi{presupposition} that I am proposing is strong enough to rule out \textit{togda} with present-under-past, but is weak enough to make it compatible with a simultaneous reading of past-under-past.
...
\noindent Under those assumptions the fact that \textit{sejčas} is acceptable in \REF{20:ex43} follows straightforwardly (with the assumption that \isi{present tense} in \ili{Russian} can be interpreted as a relative present).\largerpage[2]

The main interest for us here is the example \REF{20:ex44} and the fact that \textit{sejčas} is not acceptable in this context. Again I will assume the \textit{de re} \isi{construal} for the simultaneous reading of past-under-past in \REF{20:ex44}. The LF that will be interpreted here, namely \figref{20:ex46}, is structurally identical to the one given in \figref{20:ex35} (but the lexical items are different).
...
\noindent In the actual world, $g(4)$ (past time from the embedded \isi{clause}) is the $z$ such that it is the 9-month interval that surrounds $g(2)$ (past time of saying). In Tanja’s alternatives Tanja is pregnant at the time $z$ such that $z$ is the 9-month interval that surrounds her local now.

Intuitively it is clear that the clash happens because \textit{sejčas} has a \isi{present tense} orientation and it is not compatible with the \isi{past tense}. However, the \isi{past tense} under the \textit{de re} analysis of the simultaneous reading of past-under-past is not interpreted in the same \isi{clause} as \textit{sejčas}, thus no clash is predicted. Moreover the presence of \textit{sejčas} in the sentence is predicted to enforce the simultaneous reading of past-under-past the way `then' enforces it, due to the fact that \textit{sejčas} picks intervals that surround the local evaluation time.
...
In \ili{English}, present-under-past in complement clauses cannot get the simultaneous reading. The \ili{English} \isi{present tense} cannot be interpreted as a relative present. The absence of the relative present reading in \REF{20:ex52} shows that the \ili{English} \isi{present tense} is sensitive to the context time and not the evaluation time. In \REF{20:ex52} present-under-past gets only the so-called double access reading.
...
\noindent Thus if we extend the analysis suggested here for the \ili{Russian} \textit{sejčas-}cases with the embedded past to \ili{English} \textit{then}{}-cases with the embedded present, the ill-formedness of \REF{20:ex53} follows without any further assumptions.


% SECTION CONCLUSION
\section{Conclusion}\label{20:sConclusion}

In this paper, I looked at simultaneous readings of present-under-past and past-under-past in complement clauses in \ili{Russian}. I have formulated the \isi{adverbial} puzzle: there are adverbials like \textit{togda} `then' that can enforce the simultaneous reading of past-under-past but are completely infelicitous with present-under-past; and there are adverbials like \textit{sejčas} `now' that are compatible with an embedded relative present, but not with past-under-past.

I suggested that the restriction on the use of \textit{togda} in \ili{Russian} can be explained if \textit{togda} carries a \isi{presupposition} that the time intervals it picks are not equal to the evaluation time. I have shown that this \isi{presupposition} is strong enough to make \textit{togda} incompatible with the relative present, however weak enough to be compatible with the simultaneous reading of past-under-past. The reason for this is that the simultaneous reading of past-under-past in \ili{Russian} is derived via \textit{de re} \isi{construal} and the meaning resulting from this \isi{construal} does not require that the two intervals are equal, it is enough for them to simply overlap.

I have demonstrated that the fact that \textit{sejčas} is felicitous with present-under-past in \ili{Russian} and is not acceptable with the simultaneous reading of past-under-past does not follow from the classic \textit{de re} analysis of simultaneous readings of past-under-past. The reason for this is that since the \isi{past tense} moves out of the embedded \isi{clause}, no meaning clash is predicted between the meaning of the present oriented \isi{adverbial} \textit{sejčas} and the \isi{past tense}. I have shown that the fact that \textit{sejčas} is infelicitous with past-under-past in \ili{Russian} follows straightforwardly if we allow it to be interpreted \textit{de re} together with an embedded \isi{past tense}. I extended this analysis to explain the fact that \textit{then} is not compatible with present-under-past in \ili{English}.\largerpage[-2]
