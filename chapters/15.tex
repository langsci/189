...
Vanessa Petroj. Extract to unravel: Left branch extraction in Romanian/Serbian code-switching. To appear in: Denisa Lenertová, Roland Meyer, Radek Šimík \& Luka Szucsich (eds.), \textit{Advances in formal Slavic linguistics 2016}. Berlin: Language Science Press. [preliminary page numbering]
}
...
The paper is organized as follows. \sectref{s2} provides the demographics, methods, and type of data used for this study. In \sectref{s3}, basic assumptions and relevant LBE background are introduced. \sectref{s4} gives the background of the relevant CS construction to introduces the main questions addressed in this paper, and \sectref{s5} investigates LBE in CS. Finally, \sectref{s6} concludes the paper and offers future research directions.
...
\noindent While I will only focus on the generalization in \REF{ex1}, I refer the reader to \citet{Boskovic2008,Boskovic2012} for a comprehensive list of generalization with discussions.
...
Moving on to concrete examples, while LBE is disallowed in English (a DP language) \REF{ex2}, and in Spanish (a DP language) \REF{ex3}, it is allowed in Serbian (an NP language) \REF{ex4}:\footnote{\label{fn3}Note that LBE is not possible with non-agreeing adjectives in Serbian (see \citealt{Boskovic2013}).}
...
\ea \label{ex3}
    ...
    \gll Supuestas$_1$ investigaba [\textsubscript{DP} t$_1$ estafas].\\
         ...
         \glt `I used to investigate alleged frauds.'  \hfill (Spanish, \citealt{Riqueros2013})
    }
	\ex[*]{ \label{ex3b}
    \gll Profesionales$_1$ ofrecía [\textsubscript{DP} traducciones t$_1$].\\
         ...
         \glt `I used to offer professional translations.' \hfill (Spanish, \citealt{Riqueros2013})
         }
	\z
\z
...
\noindent When this is applied to Romanian and Serbian, the outcome is clear. Serbian (NP) allows LBE as in \REF{ex6}, and Romanian (DP) disallows it, as in \REF{ex7}:
...
\noindent In this construction, the elements that participate in CS are the Romanian definite article \textbf{-ul}, the Serbian noun \textit{ispit,} and the Serbian adjective \textit{teški}. The counterparts of Romanian and Serbian constructions are illustrated below in \REF{ex11a} and \REF{ex11b} respectively:
...
\noindent Being either an NP or a DP language has additional consequences. In this case, it means different ways in which a language can express definiteness. Specifically, while Romanian expresses definiteness through definite articles on nouns \REF{ex12a} or adjectives \REF{ex12b}, Serbian has an alternative way of obtaining definite versus indefinite interpretation. As illustrated in \tabref{t1}, Serbian has two lexical forms for adjectives: short form (\textsc{sf}) and long form (\textsc{lf}). These two forms are considered by some authors \citep{Aljovic2002,Despic2011,Talic2014} to correspond to definite/specific \REF{ex13a} and indefinite/non-specific \REF{ex13b} interpretations, respectively.\footnote{\label{fn6}For current purposes, I will simplify matters a bit and will consider the long vs. short form contrast to impose a definite vs. indefinite NP interpretation, respectively. For relevant discussion, see \citet{Aljovic2002,Despic2011,Talic2014,Stankovic2015}; a.o.}
...
As LBE is a reliable test for identifying the DP/NP parameter setting of a natural language, the same test is applied to CS constructions that include structures like \REF{ex10}, repeated below as \REF{ex10x}.
...
\noindent Recall that as predicted by the generalizations in \citet{Boskovic2008}, Romanian, being a DP language, disallows LBE and Serbian, an NP language, allows it. This is illustrated in \REF{ex6} for Serbian and in \REF{ex7} for Romanian, repeated below as \REF{ex13x} and \REF{ex14x}, respectively:
...
The paradigm below starts with \REF{ex16}, in which CS occurs within a TNP where the verb is Romanian, the definite article is Romanian, and the noun and the adjective are Serbian. As illustrated in \REF{ex16b}, LBE out of this TNP is disallowed. In \REF{ex17}, the verb is still Romanian, but even a fully Serbian TNP fails the LBE test. Interestingly, when the Romanian verb is replaced by its Serbian counterpart in \REF{ex18}, LBE improves drastically. Interestingly, while the Serbian verb \textit{can} take a DP complement in \REF{ex19a}, extraction of the adjective is blocked in \REF{ex19b}, confirming that \textbf{-ul} may indeed point towards the existence of the DP layer.\footnote{\label{fn9}I would like to thank an anonymous reviewer for noticing the incomplete paradigm and pointing out the relevance of the example in \REF{ex18}.}
...
\noindent Based on the above discussion, I take (dis)allowing LBE to indicate the presence or absence of the DP layer. The ungrammaticality of \REF{ex17b} and \REF{ex19b} then indicates that any Romanian element in the VP domain forces DP-hood on the object. What is particularly interesting here is that although the entire TNP is in Serbian, LBE still cannot take place. This suggests that although no Romanian D element is present overtly, there is still a DP projection here, which is not the case in \REF{ex18}, where LBE improves drastically with a Serbian verb introduced in the structure. Additionally, the paradigm in \REF{ex16}--\REF{ex19} confirms that regardless of the verb being Romanian or Serbian, the presence of a Romanian element in the object position will always have the DP layer.

Given that both Romanian and Serbian verbs can occur and take either a Romanian or a Serbian complement in CS, data from above indicates that Romanian verbs must take a DP complement even in CS as in \REF{ex20a}, while a Serbian verb can take either an NP complement as in \REF{ex18b}, or a DP complement, as in \REF{ex20b}.
...
\noindent We then have the generalization in \REF{ex21}:\footnote{\label{fn10}The pattern of certain elements allowing DP or NP arguments seems to extend beyond the VP domain, specifically, with respect to CS of conjuncts and coordinated structures. I refer the reader to \citet{Petroj} for more examples and more detailed explanation.}
...
\noindent I will now test the LBE of adjectives out of a ditransitive construction. Examples in \REF{ex22} and \REF{ex24} represent fully Serbian sentences with the LBE of the possessor out of the indirect object (IO) in \REF{ex22b} and direct object (DO) in \REF{ex24b}. As expected, Serbian being an NP language, LBE is allowed in both cases. In contrast, when a Romanian object is introduced into the structure in \REF{ex23} and \REF{ex25}, LBE out of the Serbian object in \REF{ex23b} and \REF{ex25b} leads to ungrammaticality.\footnote{\label{fn11}\textbf{Pe} in \REF{ex23} is a dummy preposition assigning the accusative to its complement. It is comparable to the Spanish \textit{a,} illustrated in \REF{fn11ex}.
...
\noindent \REF{ex23} and \REF{ex25} show that when one object is in Romanian and the other in Serbian, LBE is not allowed even when the LBE is attempted out of the TNP that contains Serbian elements only. This is especially interesting since LBE was allowed once a Serbian verb was introduced into the structure in \REF{ex18}. \REF{ex23} and \REF{ex25} indicate that any Romanian element (not just the verb) in the \textit{v}P/VP domain blocks LBE. With respect to the DP/NP status, it seems like both objects are DPs when one object is in Romanian. These examples then indicate that no structural mixing regarding the categorical status is allowed between the objects in a double object constructions (where one object would be an NP and one object a DP); if one object is a DP, both must be DPs. Consequently, if \textit{v}P is considered a phase, the following generalization can be made:\footnote{\label{fn12}An anonymous reviewer pointed out an interesting question about the generalization in \REF{ex26}, namely, that having a Romanian low/VP-adjunct after a Serbian ditransitive construction like the one in \REF{fn12ex} might reveal additional (counter)evidence for the structure of mixing within the spell-out domain. While the sentence in \REF{fn12exb} is only marginally acceptable, the subjects reported challenges in processing the sentence, rather than in grammaticality, which can be assigned to prosodic factors of a fully Serbian LBE construction. I leave CS of adjuncts for future research.

\ea\label{fn12ex}\ea[]{\gll \textit{Moja} \textit{drugarica} \textit{šalje} \textit{svoju} \textit{knjigu} \textit{mom} \textit{bratu} \textbf{cu} \textbf{avion-ul}.\\
...
\glt ‘My friend sends her book to my brother via plane.’\label{fn12exb}}
\z\z
...
In the examples below, \REF{ex27} represents a fully-Serbian example, with the possessor being extracted from the subject in \REF{ex27b}. This being a fully Serbian construction, LBE is allowed.
...
\noindent Interestingly, when a Romanian element is introduced as the DO in \REF{ex28} and as the IO in \REF{ex30}, LBE out of a fully-Serbian Subject is permitted in both cases, as in \REF{ex28b} and \REF{ex30b}.
...
\noindent These data contrast with \REF{ex23} and \REF{ex25} where the introduction of a Romanian internal argument blocked LBE out of the other internal argument. In contrast, LBE out of the subject is not affected by CS in the internal arguments of the verb. Based on these examples, the following generalizations can be made:
...
\noindent Notice also that a Romanian external DP argument does not force DP-hood on a Serbian internal argument, as indicated by the possibility of LBE in \REF{ex33}:
...
We can assume then that the \textit{v}p/VP spell-out domain may look something like \REF{ex34}, whereby CS below the \textit{v}P-level affects the entire phasal domain, but not the area above it:
