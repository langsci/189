...
\begin{document}
\maketitle
\shorttitlerunninghead{Unifying structural and lexical \isi{case assignment} in Dependent Case Theory}
...
\noindent We conclude from these tests that the base \isi{word order} of objects in \ili{Serbian} is IO \before{} DO. 
...
\noindent The similarities between \REF{16:structuraladjust} and \REF{16:lexicaladjust} above can be captured by the derivations in \REF{16:ex31} and \REF{16:ex32}. While verbs with `structural' \isi{dative} contain an overt DP as a DO, \textit{adjust}-type verbs contain a silent DP. Crucially, the \lr{} feature is assigned to the higher of the two DPs in the VP. While in \REF{16:ex31} the lower DP receives the \hr{} feature and thereby \accc{} case upon merging the \isi{external argument}, in \REF{16:ex32}, the lower DP argument in the VP is reduced (or alternatively it starts out as a null DP) and becomes realised by \textit{se}.
