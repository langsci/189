\documentclass[output=paper,modfonts,hidelinks,newtxmath
...
\noindent The data clearly demonstrate that \isi{adverbial} ENs and DNs differ with respect to their semantic type and scopal properties. The following sections will explore some additional semantic phenomena related to \isi{adjectival} and nominal forms of DNs.
...
\noindent Furthermore, we assume the standard syntactic analysis of comparatives. In particular, we adopt the so-called small DegP view on which the comparative marker \textit{-er} and the than-\isi{clause} form a constituent at LF and the entire DegP serves as an argument of the gradable predicate (e.g., \citealt{heim_degree_2000}), as illustrated in \REF{tree-small-DegP}. 

\ea \Tree[.AP [.DegP [.Deg \textit{-er} ] [.PP {[\textit{than}\ldots]} ] ] [.A \textit{tall} ]]\label{tree-small-DegP}
\z
...
\textsc{cnc}&\ili{Czech} National Corpus\\
\textsc{comp}&comparative\\
\textsc{dn}&degree \isi{numeral}\\
\textsc{en}&event \isi{numeral}\\
\end{tabularx}%
\begin{tabularx}{.5\textwidth}{@{}lQ@{}}
\textsc{eq}&\isi{equative}\\
\textsc{ins}&\isi{instrumental}\\
\textsc{(i)pfv}&(im)\isi{perfective}\\
\textsc{refl}&reflexive \isi{pronoun}\\
\end{tabularx}
