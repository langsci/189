...
In \ili{Russian}, most verbs come in \isi{aspectual} pairs \citep[371]{Mezhevich2008} – an \isi{imperfective} form and corresponding \isi{perfective} form – and tense is often defined with respect to aspect \citep[373]{Mezhevich2008}. In the indicative mood (that of “independent main assertive \isi{clause} type[s]” \citep[1]{Wiltschko}), \isi{imperfective aspect} allows for temporal distinctions among past, present, and a periphrastic future; \isi{perfective} only allows for past and future readings \citep[371]{Mezhevich2008}. Among non-past forms, aspect plays a role in distinguishing present from future. The examples in \REF{10:ex:ipfv} and \REF{10:ex:pfv} shows the temporal-\isi{aspectual} realizations for the verb ‘fall’, illustrating the \ili{Russian} tense system.
...
\noindent Unlike matrix \isi{subjunctive} clauses, a past-tense reading is unavailable for a \isi{subjunctive} \isi{complement clause}, as shown in \REF{10:ex15c}; while present and future interpretations are possible, as shown in \REF{10:ex15a} and \REF{10:ex15b}. 
...
\noindent \citet{Wiltschko} argues that in Upper Austrian \ili{German} there is a subjunctive--indicative contrast active where a tensed language, for example Standard \ili{German}, would employ the past-non-past distinction. For example, as shown in \REF{10:ex20}, \isi{subjunctive} morphology appears on the verb, closer than agreement marking.
...
The specification of [Proposition] contrasts propositions from bare events or states. [Finite] is a syntactic feature that licenses \isi{nominative} case and verbal agreement. [Deixis] anchors a \isi{clause} to the moment of speech. [Modality] carries the \isi{semantics} of necessity or possibility. [Precedence] encodes the meaning of past versus non-past. [Event] encodes the eventive (as opposed to \isi{stative}) property of a predicate. Finally, the specification of [Interval] derives imperfectivity versus perfectivity. These features are realized on multiple functional heads which together constitute the inflectional domain of the \isi{clause}.
...
Under \citeauthor{Cowper2010}’s proposal, \ili{English} modals merge in M(od) and subsequently move to T. TP, accordingly, is the projection of the feature [Proposition] given that only in propositions may the past/non-past distinction be realized. The viewpoint aspect features are realized in EP, which is not projected in \isi{stative} clauses \citep[2]{Cowper2010}. Moreover, the EPP is a property of the domain as a whole and is instantiated by the highest Infl head projected.
...
If indeed [Coin] in Force creates a barrier for inter-clausal operations like temporal anchoring, then we can explain why \isi{subjunctive} complement clauses embedded under a non-past \isi{matrix clause} cannot receive a \isi{past tense} interpretation. \REF{10:ex33}, repeated from \REF{10:ex15}, shows that a \isi{past tense} \isi{subjunctive} \isi{clause} under a non-past \isi{matrix verb} can receive a present or future reading but not a past one.
...
\noindent I claim that \ili{Russian} present and future tense forms both spellout [Coin], hence their similar morphological forms. Their interpretation as present or future arises from their \isi{aspectual} properties. The future reading in \REF{10:ex33a} is therefore licit, since nothing featurally blocks the reading.
