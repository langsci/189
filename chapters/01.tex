...
\begin{document}
\maketitle
\shorttitlerunninghead{Doubly filled COMP in \ili{Czech} and \ili{Slovenian} interrogatives}
...
\newpage 
\noindent The sentence in \REF{kdoze} is an appropriate reaction to a statement such as `Peter arrived'. The sentence in \REF{kdozeembedded} is the embedded version thereof; its markedness stems from the fact that it is relatively difficult to find contexts in which an embedded echo is felicitous. As far as the status of \textit{že} is concerned, I follow \citet{kaspar2015} in assuming that this element is located in C;\footnote{As \citet{kaspar2015} shows, there is in fact more than one \textit{že} element in \ili{Czech}, see also \citet{gruetskrabalova2012}; I will only concentrate on the declarative \isi{complementiser} appearing in the clauses under scrutiny.} hence, its co-occurrence with the wh-element in SpecCP makes the doubly filled COMP effect possible.
...
%$[Q]$
, only in \ili{Germanic}, see \REF{treewhetherthat}, while in \ili{Slavic} the question is merely echo, see \REF{treeceda}. Further, the \isi{complementiser} is inserted in certain dialects in \ili{Germanic} to lexicalise [fin], while \ili{Slavic} complementisers are inserted to type the \isi{clause}.
