...
\begin{document}
\maketitle
\shorttitlerunninghead{Gender encoding on hybrid nouns in \ili{Bosnian}/\ili{Croatian}/Serbian}
...
(i) \isi{natural gender} (reflecting the gender of the \isi{referent}) and
(ii) \isi{grammatical gender} (assigned arbitrarily). In this paper, we focus only on the Class II hybrid nouns in BCS (ending in \textit{-a}) which have grammatical \isi{feminine} gender, but variable \isi{natural gender}, which depends on the discourse \isi{referent}.
...
\noindent For instance, with a \isi{masculine} \isi{referent}, the \isi{noun} \textit{mušterija} (`customer') can trigger either grammatical \isi{feminine}  \REF{14:ex1a}/\REF{14:ex1b} or natural \isi{masculine} agreement  \REF{14:ex1c}/\REF{14:ex1d} (subject to speaker variability).
...
\noindent With female referents, however, such nouns can only trigger \isi{feminine} agreement:
...
\noindent One could treat these as so-called ``epicene'' nouns of the type found e.g. in \ili{Brazilian Portuguese} and \ili{Greek}, which can be used with both \isi{masculine} and \isi{feminine} referents without change in form \citep{bobaljikzocca,merchant14,kramerbook}.
 It has been proposed that such nouns are simply listed in the lexicon twice, each with a different \isi{gender feature} \citep[e.g.][19]{merchant14}. However, such an approach to BCS hybrid nouns seems to be problematic, as there is evidence that these nouns can \textit{simultaneously} bear natural and \isi{grammatical gender}. 
 For example, in \REF{14:ex3} the \isi{adjective} and determiner target different gender values of the \isi{noun}, while \REF{14:ex4} shows the same for the nominal modifier and the \isi{participle}.\footnote{Relative pronouns can also show different gender values than the attributive modifiers of a hybrid \isi{noun}, as shown in \REF{14:exfn1i}, obtained at \url{http://vukajlija.com/seoski-fudbalski-tim-iz-beton-lige} $<$accessed 26.11.2016$>$.
...
There is a growing body of literature on the permissibility of gender mismatches under NP \isi{ellipsis}  \citep[e.g.][]{nuneszocca10,bobaljikzocca,merchant14,sudospathas-sub20,barrie-cayuga16}.
Based on whether gender mismatches under NP \isi{ellipsis} are allowed or not, previous literature has identified three classes of nouns. The first type are the \textsc{two-way alternating nouns} (henceforth: the \textit{doctor}-class), where a \isi{masculine} antecedent can license deletion of a \isi{noun} with a \isi{feminine} \isi{referent}, and \textit{vice versa:}
...
The aim of the experiment was to discover whether an agreement mismatch was tolerated when \isi{masculine} agreement was found either the antecedent \REF{14:ex8b} or the \isi{ellipsis site} \REF{14:ex8c}, i.e. whether the hybrid nouns under study were two-way alternating, one-way alternating, or allowed no alternation at all when the \isi{noun} had natural \isi{masculine} gender  (indicated by the agreement on the \isi{adjective}). Considering previous theories \citep{nuneszocca10,bobaljikzocca,merchant14,sudospathas-sub20}, the (im)possibility of a mismatch should be an indicator of (i) the difference in the quality of gender features (i.e. differences in ther \isi{semantics} or morphosyntactic representation) and (ii) the licensing conditions on \isi{ellipsis} (i.e. identity requirements between the antecedent and the \isi{ellipsis site}). Finally, sentences in which both adjectives show \isi{feminine} agreement, regardless of the gender of the subject, were expected to be grammatical, as the hybrid nouns in question have \isi{grammatical gender} as a formal feature. 

In order to verify the grammaticality of gender mismatches under NP \isi{ellipsis}, we ran an online acceptability judgement study.\footnote{For a detailed description of the design and for all the materials see \url{https://osf.io/r3npz}.} 
The experimental design involved the factors  in \REF{14:ex9}. The first factor involves the type of agreement the \isi{adjective} has with a \isi{masculine} subject and has two levels (grammatical agreement \textsc{mf} vs. natural agreement \textsc{mm}) \REF{14:ex9a}, which should serve as an indicator of the type of gender on the hybrid \isi{noun} (grammatical vs. natural).
The second factor pertains to agreement with \isi{feminine} subjects and has only a single level (\textsc{ff}, both grammatical and natural agreement) \REF{14:ex9b}. The final factor regards the position of the \isi{masculine} \isi{referent} and has two levels:
in the first \isi{clause}, or in  the second \isi{clause} (the one with NP \isi{ellipsis}) \REF{14:ex9c}, which should indicate whether \isi{natural gender} can function as an antecedent for \isi{ellipsis} and whether it can be found in the \isi{ellipsis site}. The $2\times 2\times 1$ combination of factors in \REF{14:ex9} yield the four experimental conditions in Table \ref{14:table:experimconditions}.
 ...
 \ea \textbf{Factors} (manipulated within items)\label{14:ex9}
  			\ea \textsc{agreement with \isi{masculine} subject}; two levels: grammatical (\textsc{mf}) and natural (\textsc{mm})\label{14:ex9a}
  			\ex \textsc{agreement with \isi{feminine} subject}; one level: \textsc{ff} \label{14:ex9b}
...
\noindent Since \isi{masculine} agreement with hybrid nouns is not accepted by all speakers, a control sentence involved only \isi{masculine} referents and agreement (i.e. the condition \textsc{mmmm}) \REF{14:ex14}.
...
\noindent Furthermore, we included the following additional controls: a grammatical baseline with \isi{feminine} agreement and all \isi{feminine} referents \textsc{ffff} \REF{14:ex15}, and an ungrammatical baseline \textsc{fmfm}, involving \isi{masculine} agreement with \isi{feminine} referents \REF{14:ex16}. 
...
Each participant saw 62 sentences (24 test items + 18 controls + 20 fillers)  
 ...
 The experiment was performed by 164 volunteers, 131 female and 33 male, aged 16--66. Participants reportedly spoke different varieties of BCS: \ili{Bosnian} (22 speakers), \ili{Croatian} (5 speakers) and \ili{Serbian} (136 speakers).
...
\figref{14:fig:all-resps} shows the distribution of responses under each condition from all participants for the baselines (first column) and our four experimental conditions (second and third column). The baselines show strong grammaticality effects: \isi{feminine} agreement with \isi{feminine} subjects was rated as grammatical, while \isi{masculine} agreement was unacceptable. A u-shaped
type of distribution for \textsc{mmmm} (\isi{masculine} agreement with \isi{masculine} subjects) suggests that although the majority of
speakers dispreferred it, some speakers clearly did find it grammatical. With mismatching subjects, \isi{feminine} agreement (\textsc{ffmf} and \textsc{mfff}) was rated as grammatical, as shown in the second column. More gradient (un)acceptability was found for  \textsc{ffmm} and \textsc{mmff} (third column).
...
\figref{14:fig:resps-final} shows the four crucial conditions just for those speakers who rated the \textsc{mmmm} baseline higher than 4. We see that the patterns with \isi{feminine} agreement throughout were overwhelmingly acceptable and grammatical to these speakers, while the conditions with mismatches show more variation and received comparably lower scores.
...
The conclusion we can draw from this is that gender mismatches are possible in either direction as long as the \isi{adjectival} agreement is \isi{feminine}. 
...
\noindent Thus, with regard to the \isi{noun} classes in \tabref{14:t2}, our hybrid nouns behave like nouns of the two-way alternating \textit{actor}-class when there is agreement in \isi{grammatical gender} (\isi{feminine}), but they behave like non-alternating nouns of the \textit{brother}-class if at least one of them bears \isi{natural gender} (\isi{masculine}).
...
\noindent Following \citet{cooper83}, it is often assumed that (natural) gender features can introduce presuppositions \citep[also see][]{sauerland03,sauerland08,heim08,kratzer09,spathas10,sudo-diss}.
		In \ili{Greek}, non-alternating nouns of the \textit{brother}-class have been claimed to contain a \isi{presupposition} about gender of the \isi{referent} \REF{14:ex21} (\citealt[19]{merchant14}, \citealt[715]{sudospathas-sub20}).
...
\noindent The elided and antecedent \isi{noun} have conflicting gender presuppositions, not mutually-entailing:	
...
\noindent		In the analysis of two-way alternating  ``epicene'' nouns, however, it is often assumed that they do not contain any lexical presuppositions 
		about gender \REF{14:ex24}, and thus \isi{ellipsis} is licensed \REF{14:doctor}.
...
\noindent If we adopt a similar approach for hybrid nouns in BCS, then hybrid nouns agreeing in \isi{grammatical gender} (\isi{feminine}) do not contain any \isi{presupposition} about gender, while the hybrid nouns that agree in \isi{natural gender} (\isi{masculine}) should have an additional gender \isi{presupposition}. Assuming that the grammatical \isi{feminine} gender of hybrid nouns does not contribute any gender \isi{presupposition}, this would make it compatible with male and female referents:
...
\noindent Returning to gender mismatches under \isi{ellipsis}, in an example such as \REF{14:ex31}, involving \isi{feminine} agreement in both clauses, both instances of \textit{mušterija} will lack \isi{natural gender} feature and the corresponding \isi{presupposition}.
...
\noindent		In order to analyze the \isi{ellipsis} patterns, we adopt the assumptions of Distributed Morphology that nouns are built up from a category neutral root and a head $n$ that categorizes this root as nominal \citep{hallemarantz93,harleynoyer99,kihm05,acquaviva09,kramerbook}. Following \citet{kramerbook}, we will treat gender for now as a feature introduced by $n$ (although see below for more detail about the different possibilities of simultaneous separate structural encoding of natural and \isi{grammatical gender} on hybrid nouns). We assume further that number features of nouns are introduced on a NumP. These assumptions yield the structure for \REF{14:ex31} as given in \REF{14:ex32}, where \isi{ellipsis} is triggered by an [E] feature on Num \citep[e.g.][]{merchant14,saabliptak16,saab-nomel}. 		
...
\noindent		The same situation holds if \isi{masculine} agreement obtains in the antecedent:
...
\noindent Since \textit{mutual} \isi{entailment} is required, the existence of \isi{masculine} gender at only one of the conjuncts results in ungrammaticality, since the e-\textsc{given}ness requirement is not met.\largerpage[2]
...
\noindent		This accounts for why mismatches in \isi{referent} gender are not tolerated if the \isi{adjective} agrees in \isi{masculine} in one \isi{conjunct} only.
...
\noindent		Exactly why \isi{masculine} agreement with hybrid nouns \REF{14:refmmmm} is not possible for all speakers is an open issue, and we suggest it could be due to not all speakers having the variant of $n$ containing the additional [\textsc{m}] feature. We leave further examination of interspeaker variation to future research.	

\subsection{The encoding of natural and grammatical gender features}
		
Since the nouns addressed in this paper have the possibility to simultaneously encode both the grammatical \isi{feminine} and the natural \isi{masculine} gender (as illustrated by \REF{14:mismatchbcs}, see also \citealt{wandz03,despichybrid17,puskar17}), a question that remains open is where exactly these features are encoded in the DP structure.		
  
Some accounts \citep{matushansky13,pesetsky14,Landau2016DPinternalsemantic} assume \isi{grammatical gender} to be encoded low in the \isi{noun}'s structure, as a property related to the nominal stem. Natural gender is optionally introduced on a higher functional projection. Under this approach, we would treat the grammatical \isi{feminine} gender of our hybrid nouns as a property of $n$, while \isi{natural gender} would be introduced at a higher functional projection (e.g. GenP, \citealt[cf.][]{picallo91}). The lower gender would not introduce any gender presuppositions, while the denotation of the gender on Gen would be a partial identity function restricting the set of customers to the set of male customers \citep{cooper83}. If the male-referring hybrid \isi{noun} triggers \isi{feminine} agreement, \isi{adjectival} concord then targets the \isi{grammatical gender} feature on $n$ \REF{14:ex41}.
...
\noindent For masculine-agreeing hybrid nouns, the closest target for Agree will be the higher \isi{masculine} gender \REF{14:ex42}. %under the assumption that 
...
\noindent If this analysis were to be adopted, the ungrammatical sentences would be ruled out by prohibiting  any mismatches between the features of antecedent and those of the \isi{ellipsis site} (cf. \citealt{merchant13}). 
			...
			To sum up, both types of approaches to the structural position of gender features would in principle be compatible with the results of our experiment. Future research should then be tasked with teasing apart the two and defining the exact locus of the natural and \isi{grammatical gender} features. However, what the results undoubtedly reveal is the necessity to represent the two types of features separately, as well as that they differ in quality, such that \isi{natural gender} has an additional meaning component.   
			
			
			\section{Conclusion}	
			
			This paper has presented new experimental data from  NP \isi{ellipsis} showing that hybrid nouns in BCS show a split behaviour with regard to gender \isi{ellipsis}: if they agree in \isi{feminine} gender, then mismatches in \isi{referent} gender are permitted for either the antecedent or elided \isi{noun}. However, \isi{masculine} agreement in either \isi{conjunct} blocks gender mismatches. We have linked this to the optional presence of a natural [\textsc{m}] \isi{gender feature} which represents a target for \isi{adjectival} concord and introduces a gender \isi{presupposition}.
			It is this gender \isi{presupposition} that destroys the \isi{mutual entailment} relation required by \citeauthor{merchant2001}'s (\citeyear{merchant2001}) e-\textsc{given}ness requirement on \isi{ellipsis} licensing.
			Consequently,  this study suggest that natural \isi{masculine} gender is syntactically absent when the \isi{adjective} agrees in \isi{grammatical gender}.
