...
\noindent While examples in (\ref{vsi}) show that there are several discourse particles that can appear in sluicing in Slovenian, we focus only on discourse particles \textit{že} and \textit{pa} here. Some initial thoughts on other particles in sluicing can be found in \cite{marusicetal2015}, but since the role of Slovenian discourse particles in wh-questions has previously not been sufficiently described, we start off by showing some properties these elements display when they are used as discourse particles (and not topic or focus particles). Both \textit{pa} and \textit{že} have many different uses and meanings, which we will discuss in \sectref{s2}. In \sectref{s3} we take examples with sluicing and the particles \textit{pa} and \textit{že} to give new arguments both against positioning these particles within the wh-phrase and to show that in addition to (complex) wh-phrases non-wh-material can also survive sluicing in Slovenian. In \sectref{s4} we discuss the position of discourse particles with respect to the  clitic cluster and the adverbs in the IP to show that discourse particles appear in the left periphery, higher than IP adverbs, confirming the earlier proposal by \cite{marusicetal2015}. \sectref{s5} concludes the paper.
...
\section{Wh-phrases, discourse particles and{\dots} what else?}\label{s3}
...
\trans `Who (threw the glasses) to whom?'}\label{kdokomu}
\ex[*]{\gll Kdo pa komu pa?\\
    	who.\textsc{nom} \textsc{pa} who.\textsc{dat} \textsc{pa}\\} \label{kdopakomupa}
\ex[]{\gll Kdo komu pa?\\
	who.\textsc{nom} who.\textsc{dat} \textsc{pa}\\
\trans `(I want to know) Who (threw the glasses) to whom?'}\label{kdokomupa}
\ex[]{ \gll Kdo pa komu?\\
	who.\textsc{nom} \textsc{pa} who.\textsc{dat} \\
\trans `(I want to know) Who (threw the glasses) to whom?'}\label{kdopakomu}
\ex[?]{\gll  Kdo pa komu to?   \\
 who.\textsc{nom}	\textsc{pa} who.\textsc{dat} \textsc{to}\\
 \trans `(I want to know) Who (threw the glasses) to whom?'}\label{kdopakomuto}
\end{xlist}\end{exe}
